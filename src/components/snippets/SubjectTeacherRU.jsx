...
const SubjectTeacherRU = ({ modalState, setModalState, teacherOptions, rowData, selectedForm, refreshTable }) => {
  ...
  const handleSubmit = async (formValues) => {
    ...
    try {
      ...
      showToast('Subject teacher updated successfully', 'success', {duration : 3000});
  ...
  return (
    <div>
      <ModalForm
        isOpen={modalState.editSubjectTeacher && !isLoading}
        onClose={() => {
          setError('');
          setTeacherId('');
          setModalState(prev => ({ ...prev, editSubjectTeacher: false }));
