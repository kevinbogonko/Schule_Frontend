...
const StaffSMS = () => {
  ...
  const [selectTeacherChecked, setSelectTeacherChecked] = useState(false);
  ...
  const columns = [
    { name: "ID", uid: "teacher_id", sortable: true },
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      if (selectedYear) {
        try {
          const response = await api.post("/staff/getteachers", {
            year: selectedYear,
          });
          setTeachersList(
  ...
  const handleTeacherCheckboxChange = (e) => {
    setSelectTeacherChecked(e.target.checked);
  };
  ...
  const handleSendSMS = async () => {
    ...
    try {
      ...
      if (!selectTeacherChecked) {
        recipients = teachersList
        ...
        recipients = teachersList
          .filter(
            (teacher) =>
              teacher.phone && selectedTeachers.includes(teacher.value)
          )
          .map((teacher) => ({
            id: teacher.value,
            number: teacher.phone,
            ...
            >
              <div className="flex flex-col space-y-4">
                ...
                {teachersList.length > 0 && (
                  <div className="mt-2">
                    <Checkbox
                      id="select-teacher-checkbox"
                      label="Select Teachers"
                      className="mb-2"
                      checked={selectTeacherChecked}
                      onChange={handleTeacherCheckboxChange}
                    />
                    {selectTeacherChecked && teachersList.length > 0 && (
                      <CheckboxGroup
                        label="Teacher List"
                        className="mt-1 mr-0"
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    disabled={sending || !message || teachersList.length === 0}
