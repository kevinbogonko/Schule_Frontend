...
const AddMark = ({syst_level}) => {
    ...
    const fetchExams = async () => {
      if (selectedForm && selectedTerm) {
        ...
        try {
          ...
          resetBelow('term');
          showToast(err.response?.data?.message || 'Failed to fetch exams', 'error', {duration : 3000, position : 'top-right'});
        }
      }
    };
    fetchExams();
  ...
  const handleUpdatePaperSetup = async () => {
    ...
    try {
      ...
      showToast('Paper setup updated successfully', 'success', {duration :3000, position : 'top-right'});
      ...
      showToast(error.response?.data?.message || 'Failed to update paper setup', 'error', {duration :3000, position : 'top-right'})
    } finally {
