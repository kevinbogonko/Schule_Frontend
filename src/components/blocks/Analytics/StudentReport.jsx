...
const StudentReport = ({ syst_level }) => {
  ...
  useEffect(() => {
    if (selectedStreams.length > 0) {
      const filteredStudents = studentList
        .filter((student) => selectedStreams.includes(student.stream_id))
        .map((student) => student.value);
      setSelectedStudentList(filteredStudents);
    }
  }, [selectedStreams, studentList]);
    ...
    const fetchStreams = async () => {
      if (selectedYear && selectedForm) {
        try {
          ...
          showToast("Failed to fetch streams", "error", { duration: 2000 });
        }
      }
    };
    ...
    const fetchStudents = async () => {
      if (selectedYear && selectedForm) {
        try {
          ...
          showToast("Failed to fetch students", "error", { duration: 2000 });
        }
      }
    };
  ...
  useEffect(() => {
    if (selectStudentChecked) {
      setSelectStreamChecked(false);
    }
  }, [selectStudentChecked]);

  useEffect(() => {
    if (selectStreamChecked) {
  ...
  const getFileName = () => {
    const examName =
      examRows.length > 1 ? examAliasSingle : examRows[0]?.exam || "report";
    return `student_report_${selectedForm}_${selectedTerm}_${examName}_${selectedYear}.pdf`;
  };
    ...
    <div className="p-0">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          <ReusableDiv
            className="ml-0 mr-0 ring-1 flex-1 bg-blue-100 dark:bg-gray-800"
            tag="Select Exam"
            icon={PiExam}
            collapsible={true}
          >
                ...
                {showStreamControls && (
                  <div className="mt-2">
                    <Checkbox
                      ...
                      onChange={(e) => setSelectStreamChecked(e.target.checked)}
                    />
                    {selectStreamChecked && (
                      <Dropdown
